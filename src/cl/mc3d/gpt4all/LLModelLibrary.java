...
public interface LLModelLibrary {
    ...
    @u_int64_t
    long llmodel_get_state_size(Pointer model);

    @u_int64_t
    long llmodel_save_state_data(Pointer model, Pointer dest);

    @u_int64_t
    long llmodel_restore_state_data(Pointer model, Pointer src);

    void llmodel_set_implementation_search_path(String path);

        // 2.7.2  no work
    void llmodel_prompt(Pointer model, @Encoding("UTF-8") String prompt,
            @Encoding("UTF-8") String prompt_template,
            PromptCallback prompt_callback,
            ResponseCallback response_callback,
            RecalculateCallback recalculate_callback,
            @In LLModelPromptContext ctx,boolean special, @Encoding("UTF-8") String fake_reply);
    // 2.7.1 it works
    void llmodel_prompt(Pointer model, @Encoding("UTF-8") String prompt,
            @Encoding("UTF-8") String prompt_template,
            PromptCallback prompt_callback,
            ResponseCallback response_callback,
            RecalculateCallback recalculate_callback,
            @In LLModelPromptContext ctx,boolean special);
//it works 2.7.0
    void llmodel_prompt(Pointer model, @Encoding("UTF-8") String prompt,
            PromptCallback prompt_callback,
            ResponseCallback response_callback,
    ...
    boolean llmodel_has_gpu_device(Pointer model);
    boolean llmodel_gpu_init_gpu_device_by_int(Pointer model, int device);
}
