...
public class UnitaryProcessPublishFile implements Runnable {
    ...
    public UnitaryProcessPublishFile(String locationStart, IgniteCache igniteCache, String cacheName, String hostname, String modelFilePath, String cipherMessage, String keyStorePass) {
    ...
    public List getLocalDocsList(String folder) {
        ...
        for (String sFile : lQuestionsFolder) {
            if (sFile.toLowerCase().endsWith(".pdf")||sFile.toLowerCase().endsWith(".txt")) {
                data.add("/"+folder + "/" + sFile);
            } else {
                String subFolder = folder + "/" + sFile;
                File fDir = new File(subFolder);
                if (fDir.isDirectory()) {
                    data.addAll(getLocalDocsList(subFolder));
                }
            }
        }
        return data;
